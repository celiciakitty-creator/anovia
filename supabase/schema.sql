...
-- ---------------------------------------------------------------------------
-- Table: comments
-- Project and task discussion threads. Exactly one target per row.
-- ---------------------------------------------------------------------------
...
CREATE INDEX comments_project_id_created_idx
  ON public.comments (project_id, created_at DESC)
  WHERE project_id IS NOT NULL;

CREATE INDEX comments_task_id_created_idx
...
COMMENT ON POLICY "comments_select_accessible_target" ON public.comments IS
  'Authenticated users can read comments when the linked project or task is visible (mirrors projects/tasks SELECT).';
...
COMMENT ON POLICY "comments_insert_own_on_accessible_target" ON public.comments IS
  'Users insert comments as themselves on projects or tasks they can access.';
...
COMMENT ON POLICY "comments_update_own" ON public.comments IS
  'Authors may update their own comments on accessible targets.';
...
COMMENT ON POLICY "comments_delete_own" ON public.comments IS
  'Authors may delete their own comments on accessible targets.';
